...
func NewReactor(mempool *TxPool, opts *ReactorOptions) (*Reactor, error) {
	...
	memR.BaseReactor = *p2p.NewBaseReactor("Mempool", memR, 100)
	return memR, nil
}
...
func (memR *Reactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	case *protomem.SeenTx:
		...
		if err != nil {
			...
			memR.Switch.StopPeerForError(e.Src, memR.String(), err)
			return
		}
	...
	default:
		...
		memR.Switch.StopPeerForError(e.Src, memR.String(), fmt.Errorf("mempool cannot handle message of type: %T", msg))
		return
	}
}

...
func (memR *Reactor) FetchTxsFromKeys(ctx context.Context, blockID []byte, compactData [][]byte) ([][]byte, error) {
	memR.Logger.Info("fetching transactions from keys", "numTxs", len(compactData))
	...
	if len(missingKeys) == 0 {
		memR.Logger.Info("compact all transactions found in mempool", "blockID", blockID)
		return txs, nil
	}
...
// FetchKeysFromTxs is in many ways the opposite method of FetchTxsFromKeys. It
// takes a full block generated by the application and reduces it to the set of
// keys that need to be gossiped from one mempool to another nodes mempool in
// order to recreate the full block.

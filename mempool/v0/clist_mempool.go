...
// Safe for concurrent use by multiple goroutines.
func (mem *CListMempool) ReapMaxBytesMaxGas(maxBytes, maxGas int64) (types.Txs, []types.TxKey) {
	mem.updateMtx.RLock()
	defer mem.updateMtx.RUnlock()
	...
	keys := make([]types.TxKey, 0, mem.txs.Len())
	for e := mem.txs.Front(); e != nil; e = e.Next() {
		memTx := e.Value.(*mempoolTx)

		txs = append(txs, memTx.tx)
		keys = append(keys, memTx.tx.Key())

		dataSize := types.ComputeProtoSizeForTxs([]types.Tx{memTx.tx})

		// Check total size requirement
		if maxBytes > -1 && runningSize+dataSize > maxBytes {
			return txs[:len(txs)-1], keys
		}
		...
		if maxGas > -1 && newTotalGas > maxGas {
			return txs[:len(txs)-1], keys[:len(txs)-1]
		}
